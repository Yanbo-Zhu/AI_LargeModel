...
▪ While the attention layer refined an embedding by looking onto its context in the input sequence, the MLP refines each contextualized embedding 𝑒റ′ Feed Forward independently of the other tokens in the input sequence 

Attention 负责融合上下文信息，
MLP 负责对每个 token 的表示进行独立的非线性变换和增强。
...
==Attention integrates contextual information,==
===MLP applies independent nonlinear transformation to each token representation.===
...
### 8.2.1 temperature  in output processing 
